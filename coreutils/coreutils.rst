...
ファイルまるまる出力系
----------------------
ファイルが高まる出力系のコマンド [#core-sec]_ です。ファイルの意識を高めましょう [#file-takamaru]_ 。
...
ログファイルから==または、192.168.0.1という文字列を抜き出しています [#taila]_ 。"=="というのは、やってみてのお楽しみ。
...
圧縮されているファイルの文字数を知りたいだけの時は、 ``bacat foo.bz2 | wc -c`` などとするのがよさそう。
