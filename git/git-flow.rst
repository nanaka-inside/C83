...
要するに、Gitブランチをどのように運用するかを決めたガイドラインの一つです。Gitは柔軟性が高く、ブランチをどのように運用するかは開発者の判断に委ねられています。しかしながら、経験が少ない初期はブランチの運用スタイルを決めるのは難しいので、最初にお手本となると手法を採用した方が良いでしょう。そのような場合はgit-flowでのブランチ運用を考えると良いでしょう。

git-flowでは基本的にセントラルリポジトリは1つとします。つまりoriginがひとつであり、その内部のmasterブランチもひとつです。
