...
thanks_cmdの文法規則はa_exprと等価ですが、アクションではSelectStmtから検索用の構文木のノードを生成し、自身がSelect文と同様の振る舞いをするよう設定を行っています。
...
今回は構文拡張をメインに扱いましたので、今度はエグゼキュータの拡張等もテーマにしていきたいなぁと考えています。
それでは！
